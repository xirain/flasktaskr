...
def weixin_echo_you_said():
    ...
    response_data = """<xml><ToUserName><![CDATA[{}]]></ToUserName><FromUserName><![CDATA[{}]]></FromUserName><CreateTime>{}</CreateTime><MsgType><![CDATA[text]]></MsgType><Content><![CDATA[{}]]></Content></xml><MsgId>{}</MsgId>""".format(to_user, from_user, int(time.time()), 'current just test, what you said is {}'.format(content), msg_id)
